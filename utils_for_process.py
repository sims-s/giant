...
if aws:
    ...
    ADDRESS_LIST = [("13.58.160.42", 5001)]
else:
    HOME = ('127.0.0.1', 5000)  
    ADDRESS_LIST = [('127.0.0.1', 5001+i) for i in range(100)]
...
class ListeningThread(threading.Thread):
    """ This thread is used by a process to listen for messages."""
    def __init__(self, parent_process):
        threading.Thread.__init__(self)
        self.parent_process = parent_process

    def run(self):
        """ This method will run all the time and listen for requests
        for resources as well as receive responses that a token has
        been granted on the default socket.
        """
